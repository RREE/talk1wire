...
package body Object.Handle.Generic_Set is
   ...
   procedure Add (Container : in out Set; Item : Object_Type_Ptr) is
   begin
      Insert (Container, Item);
   end Add;

   procedure Add (Container : in out Set; Item : Handle) is
   begin
         ...
         begin
            ...
            if 0 /= Source.Size then
               ...
               for Item in 1..Source.Size loop
                  Increment_Count (Source.Vector (Item).all);
               end loop;
            end if;
   ...
   begin
      loop
         This := (From + To) / 2;
         if Equal (Item, Vector (This).all) then
            return This;
         elsif Less (Item, Vector (This).all) then
            if This - From <= 1 then
               return -This;
            end if;
            To := This;
   ...
   function Find (Container : Set; Item : Object_Type_Ptr)
      return Integer is
   begin
      if Is_Empty (Container) or else Item = null then
         return -1;
      else
         return
            Find
         ...
         if not (Is_Empty (Left) or else Is_Empty (Right)) then
            ...
            begin
               for Index in 1..Size1 loop
                  if Find (Vector2, Size2, Vector1 (Index).all) > 0 then
